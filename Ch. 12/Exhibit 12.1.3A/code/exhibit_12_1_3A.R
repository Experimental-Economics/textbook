...
prek_avg <- mean((lower_bound_data %>% filter(d_i == 1))$std_cog_sl, na.rm = TRUE)
ctrl_avg <- mean((lower_bound_data %>% filter(d_i == 0))$std_cog_sl, na.rm = TRUE)


# --- Create plot -------------------------------------------------------------
horowitz.manski.lower.plot <- ggplot(
...
# --- Save output -------------------------------------------------------------
output_file <- file.path(output_dir, "exhibit_12_1_3A_hm_bounds_lower.png")
ggsave(output_file, plot = horowitz.manski.lower.plot, width = 10, height = 6, units = "in", dpi = 300)

cat("\n", strrep("=", 80), "\n", sep = "")
cat("EXHIBIT 12.1.3A: Horowitz and Manski Bounds (Lower) - Plot saved\n")
cat(strrep("=", 80), "\n", sep = "")
