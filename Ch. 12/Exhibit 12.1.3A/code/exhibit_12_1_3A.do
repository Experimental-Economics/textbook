...
quietly summarize std_cog_sl_lower if d_i == 1
local prek_avg_lower = r(mean)

quietly summarize std_cog_sl_lower if d_i == 0
...
twoway ///
    (kdensity std_cog_sl_lower if d_i == 0, ///
        color("255 127 127") lwidth(medium) kernel(gaussian) bwidth(0.3)) ///
    (kdensity std_cog_sl_lower if d_i == 1, ///
    ...
    title("Horowitz and Manski Bounds (Lower)", size(medlarge) color(black)) ///
    ...
    scheme(s2color) ///
    note("Lower bound: Treatment attritors assigned {&minus}3, Control attritors assigned +3" ///
         "Dashed lines show group means: Pre-K = `=string(`prek_avg_lower', "%4.3f")', Control = `=string(`ctrl_avg_lower', "%4.3f")'", ///
         size(vsmall))

// Save plot
graph export "`output_dir'/exhibit_12_1_3A_hm_bounds_lower.png", ///
...
display _newline
display "{hline 80}"
display "EXHIBIT 12.1.3A: Horowitz and Manski Bounds (Lower) - Plot saved"
display "{hline 80}"
display "Saved to: `output_dir'/exhibit_12_1_3A_hm_bounds_lower.png"
display "Pre-K Average: " %6.3f `prek_avg_lower'
display "Control Average: " %6.3f `ctrl_avg_lower'
