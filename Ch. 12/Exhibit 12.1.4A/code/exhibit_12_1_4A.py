...
data_12_1_4A = pd.read_stata(DATA_DIR / "unique_data_clean_main_synthetic.dta")
# To use actual CHECC data, comment out the line above and uncomment the line below:
# data_12_1_4A = pd.read_stata(DATA_DIR / "unique_data_clean_main.dta")


data_12_1_4A['year'] = pd.to_numeric(data_12_1_4A['year'], errors='coerce')

# Apply filters: year >= 2012, treatment in {control, prek}, exclude kinderprep
# and late randomized, require baseline cognitive score
cleaned_data_12_1_4A = data_12_1_4A[
    (data_12_1_4A['year'] >= 2012) &
    ((data_12_1_4A['treatment'] == 'control') |
     (data_12_1_4A['treatment'] == 'prek')) &
    (data_12_1_4A['kinderprep'] == 0) &
    (data_12_1_4A['late_randomized'] == 0) &
    (data_12_1_4A['has_cog_pre'] != 0)
].copy()

# Create block variable (prioritize 2012 block, fallback to 2013)
cleaned_data_12_1_4A['block'] = cleaned_data_12_1_4A.apply(
    lambda row: row['block_2012'] if row['block_2012'] != "" else row['block_2013'],
    axis=1
)

# Set summer loss cognitive score to NA when not observed
cleaned_data_12_1_4A['std_cog_sl'] = cleaned_data_12_1_4A['std_cog_sl'].where(
    cleaned_data_12_1_4A['has_cog_sl'] != 0, pd.NA
)

# Treatment indicator: 1 if pre-K, 0 if control
cleaned_data_12_1_4A['d_i'] = (cleaned_data_12_1_4A['treatment'] == "prek").astype(int)

# Response indicator: 1 if outcome observed, 0 if attrited
cleaned_data_12_1_4A['r_i'] = cleaned_data_12_1_4A['std_cog_sl'].notna().astype(int)
...
upper_bound_data_12_1_4A = cleaned_data_12_1_4A.copy()
upper_bound_data_12_1_4A.loc[
    (upper_bound_data_12_1_4A['r_i'] == 0) & (upper_bound_data_12_1_4A['d_i'] == 1),
    'std_cog_sl'
] = UPPER_BOUND
upper_bound_data_12_1_4A.loc[
    (upper_bound_data_12_1_4A['r_i'] == 0) & (upper_bound_data_12_1_4A['d_i'] == 0),
    'std_cog_sl'
] = LOWER_BOUND
...
prek_avg_upper = upper_bound_data_12_1_4A[
    upper_bound_data_12_1_4A['d_i'] == 1
]['std_cog_sl'].mean()
ctrl_avg_upper = upper_bound_data_12_1_4A[
    upper_bound_data_12_1_4A['d_i'] == 0
]['std_cog_sl'].mean()
...
plt.axvline(x=ctrl_avg_upper, color='red', linestyle='--', linewidth=0.8,
            label=f'Control Mean: {ctrl_avg_upper:.3f}')

# Labels and title
plt.xlabel('Cognitive Test Score after Summer Loss', fontsize=11)
plt.ylabel('Density', fontsize=11)
plt.title('Horowitz and Manski Bounds (Upper)', fontsize=13, fontweight='bold')
...
# Save plot
output_file = OUTPUT_DIR / 'exhibit_12_1_4A_hm_bounds_upper.png'
...
# --- Print results -----------------------------------------------------------
print("\n" + "=" * 80)
print("EXHIBIT 12.1.4A: Horowitz and Manski Bounds (Upper) - Plot saved")
print("=" * 80)
print(f"Saved to: {output_file}")
print(f"Pre-K Average: {prek_avg_upper:.3f}")
print(f"Control Average: {ctrl_avg_upper:.3f}")
