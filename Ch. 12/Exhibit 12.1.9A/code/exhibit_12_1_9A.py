...
data_12_1_9A = pd.read_stata(DATA_DIR / "unique_data_clean_main_synthetic.dta")
# To use actual CHECC data, comment out the line above and uncomment the line below:
# data_12_1_9A = pd.read_stata(DATA_DIR / "unique_data_clean_main.dta")



data_12_1_9A['year'] = pd.to_numeric(data_12_1_9A['year'], errors='coerce')

# Apply filters: year >= 2012, treatment in {control, prek}, exclude kinderprep
# and late randomized, require baseline cognitive score
cleaned_data_12_1_9A = data_12_1_9A[
    (data_12_1_9A['year'] >= 2012) &
    ((data_12_1_9A['treatment'] == 'control') |
     (data_12_1_9A['treatment'] == 'prek')) &
    (data_12_1_9A['kinderprep'] == 0) &
    (data_12_1_9A['late_randomized'] == 0) &
    (data_12_1_9A['has_cog_pre'] != 0)
].copy()

# Create block variable (prioritize 2012 block, fallback to 2013)
cleaned_data_12_1_9A['block'] = cleaned_data_12_1_9A.apply(
    lambda row: row['block_2012'] if row['block_2012'] != "" else row['block_2013'],
    axis=1
)

# Set summer loss cognitive score to NA when not observed
cleaned_data_12_1_9A['std_cog_sl'] = cleaned_data_12_1_9A['std_cog_sl'].where(
    cleaned_data_12_1_9A['has_cog_sl'] != 0, pd.NA
)

# Treatment indicator: 1 if pre-K, 0 if control
cleaned_data_12_1_9A['d_i'] = (cleaned_data_12_1_9A['treatment'] == "prek").astype(int)

# Response indicator: 1 if outcome observed, 0 if attrited
cleaned_data_12_1_9A['r_i'] = cleaned_data_12_1_9A['std_cog_sl'].notna().astype(int)


# --- Calculate trimming parameters -------------------------------------------
# Calculate response rates by treatment group
response_rate_control = cleaned_data_12_1_9A.loc[
    cleaned_data_12_1_9A['d_i'] == 0, 'r_i'
].mean()
response_rate_prek = cleaned_data_12_1_9A.loc[
    cleaned_data_12_1_9A['d_i'] == 1, 'r_i'
].mean()
...
quantile_threshold = cleaned_data_12_1_9A.loc[
    cleaned_data_12_1_9A['d_i'] == 0, 'std_cog_sl'
].quantile(trimming_fraction)

# Keep control observations above threshold (trim bottom)
control_trimmed = cleaned_data_12_1_9A[
    (cleaned_data_12_1_9A['d_i'] == 0) &
    (cleaned_data_12_1_9A['std_cog_sl'] > quantile_threshold)
]

# Combine trimmed control group with full treatment group
lower_bound_data = pd.concat(
    [control_trimmed, cleaned_data_12_1_9A[cleaned_data_12_1_9A['d_i'] == 1]],
    ignore_index=True
)


# --- Calculate summary statistics --------------------------------------------
prek_avg_lower = lower_bound_data.loc[
    lower_bound_data['d_i'] == 1, 'std_cog_sl'
].mean(skipna=True)
ctrl_avg_lower = lower_bound_data.loc[
    lower_bound_data['d_i'] == 0, 'std_cog_sl'
].mean(skipna=True)
...
plt.axvline(x=ctrl_avg_lower, color='red', linestyle='--', linewidth=0.8,
            label=f'Control Mean (trimmed): {ctrl_avg_lower:.3f}')

# Labels and title
plt.xlabel('Cognitive Test Score after Summer Loss', fontsize=11)
plt.ylabel('Density', fontsize=11)
plt.title('Lee Bounds (Lower)', fontsize=13, fontweight='bold')
...
# Save plot
output_file = OUTPUT_DIR / 'exhibit_12_1_9A_lee_bounds_lower.png'
...
# --- Print results -----------------------------------------------------------
print("\n" + "=" * 80)
print("EXHIBIT 12.1.9A: Lee Bounds (Lower) - Plot saved")
print("=" * 80)
...
print(f"Control Average (trimmed): {ctrl_avg_lower:.3f}")
