...
# Find quantile threshold for trimming (keep values above this threshold)
mean.outcome.treatment <- mean((cleaned_data %>% filter(d_i == 1))$std_cog_sl, na.rm = TRUE)

# Trim from BOTTOM: keep control observations above trimming.fraction quantile
step3 <- cleaned_data %>%
  filter(
    d_i == 0 &
    std_cog_sl > quantile(
...
mean.righttrimmed.control <- mean(step3$std_cog_sl, na.rm = TRUE)
lower_bound <- mean.outcome.treatment - mean.righttrimmed.control

# Combine trimmed control group with full treatment group
righttrimmed <- rbind(step3, cleaned_data %>% filter(d_i == 1))


# --- Calculate summary statistics --------------------------------------------
prek_avg <- mean((righttrimmed %>% filter(d_i == 1))$std_cog_sl, na.rm = TRUE)
ctrl_avg <- mean((righttrimmed %>% filter(d_i == 0))$std_cog_sl, na.rm = TRUE)


# --- Create plot -------------------------------------------------------------
lee.righttrimmed.plot <- ggplot(
  data = righttrimmed,
...
# --- Save output -------------------------------------------------------------
output_file <- file.path(output_dir, "exhibit_12_1_9A_lee_bounds_lower.png")
ggsave(output_file, plot = lee.righttrimmed.plot, width = 10, height = 6, units = "in", dpi = 300)

cat("\n", strrep("=", 80), "\n", sep = "")
cat("EXHIBIT 12.1.9A: Lee Bounds (Lower) - Plot saved\n")
cat(strrep("=", 80), "\n", sep = "")
...
cat(sprintf("Treatment Effect (Lower Bound): %.3f\n", lower_bound))
cat(strrep("=", 80), "\n", sep = "")
