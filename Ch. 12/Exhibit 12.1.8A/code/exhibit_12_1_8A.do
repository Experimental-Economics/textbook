...
// Find quantile threshold for trimming (1 - trimming_fraction)
_pctile std_cog_sl if d_i == 0, percentiles(`=100*(1 - `trimming_fraction')')
local quantile_threshold = r(r1)

// Create indicator for observations to keep after trimming
gen keep_upper = 1

// For control group: Keep observations below threshold (trim top)
replace keep_upper = 0 if d_i == 0 & std_cog_sl >= `quantile_threshold' & !missing(std_cog_sl)

// Keep only non-trimmed observations for analysis
preserve
keep if keep_upper == 1
...
twoway ///
    ...
    title("Lee Bounds (Upper)", size(medlarge) color(black)) ///
    ...
    scheme(s2color) ///
    note("Upper bound: Trimmed `=string(`trimming_fraction'*100, "%4.1f")'% from top of control distribution" ///
         "Response rates: Control = `=string(`response_rate_control', "%4.3f")', Pre-K = `=string(`response_rate_prek', "%4.3f")'" ///
         "Dashed lines: Pre-K = `=string(`prek_avg_upper', "%4.3f")', Control (trimmed) = `=string(`ctrl_avg_upper', "%4.3f")'", ///
         size(vsmall))

// Save plot
graph export "`output_dir'/exhibit_12_1_8A_lee_bounds_upper.png", ///
...
display _newline
display "{hline 80}"
display "EXHIBIT 12.1.8A: Lee Bounds (Upper) - Plot saved"
display "{hline 80}"
display "Saved to: `output_dir'/exhibit_12_1_8A_lee_bounds_upper.png"
...
display "Control Average (trimmed): " %6.3f `ctrl_avg_upper'
