...
def run_single_iteration(params):
    ...
    Parameters:
    - params: Tuple of (baseline, treatment_effect, individual_sd, error_sd,
              n_subjects, n_periods, design)

    Returns:
    - is_significant: Boolean
    """
    baseline, treatment_effect, individual_sd, error_sd, n_subjects, n_periods, design = params
    ...
    # Test significance
    return test_significance(data, design)
...
if __name__ == '__main__':
    ...
    # Create Pool once and reuse (much faster than creating/destroying for each condition)
    with Pool() as pool:
        ...
        for treatment_effect in TREATMENT_EFFECTS:
            ...
            for n_subjects in SAMPLE_SIZES:
                ...
                for design in ['WS', 'BS']:
                    ...
                    params_list = [
                        (BASELINE_MEAN, treatment_effect, INDIVIDUAL_SD, ERROR_SD,
                         n_subjects, TIME_PERIODS, design)
                        for _ in range(N_ITERATIONS)
                    ]

                    # Run iterations in parallel with optimized chunk size
                    results_list = pool.map(run_single_iteration, params_list, chunksize=50)

                    # Calculate power
                    significant_count = sum(results_list)
